...
```{sql connection="db"}
select Violation_Description, count(*) from latix
...
```{sql connection="db"}
select Issue_Year, sum(Fine_Amount) as Total_Fine from latix
where Issue_Year = '2015' or Issue_Year = '2016'
group by Issue_Year
...
```{sql connection="db"}
select Issue_Year, Location, sum(Fine_Amount) as Total_Fine from latix
