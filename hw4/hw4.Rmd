...
```{r}
#fit a linear reg model of arrdelay on distance, carriers, month, dest, origin
model_data <- flights_tbl %>%
  ...
  select(year, month, distance, uniquecarrier, description, 
         dest, origin, arrdelay)

#head(model_data)

#summarize data by carrier, month, dest, origin
model_data %>%
  group_by(uniquecarrier, month, dest, origin) %>%
  summarize(description = min(description, na.rm = TRUE), 
            distance = mean(distance, na.rm = TRUE),
            arrdelay = mean(arrdelay, na.rm = TRUE)) %>%
  select(uniquecarrier, description, distance, month, dest, origin, arrdelay) %>%
  arrange(arrdelay)
...
model1 <- model_partition$train %>%
  ml_linear_regression(arrdelay ~ 
                         distance + uniquecarrier + month + dest + origin)
